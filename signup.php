...
if(isset($_POST['submitted'])){
	...
	if(empty($_POST['fname']) && empty($_POST['lname']) && empty($_POST['username']) && empty($_POST['email']) && empty($_POST['address']) && empty($_POST['phone']) && empty($_POST['password']) && empty($_POST['rpassword'])){
        ...
        if(!$uppercase || !$lowercase || !$number || !$specialChars || strlen($password) < 8) {
           echo "<p align='middle'>Register fail,password should be at least 8 characters in length and should include at least one upper case letter, one number, and one special character.</p>";
		    ...
		    echo "<p align='middle'>Email is already use</p>";
	        }else{
			...
			$query = "INSERT INTO users(Username, Password, First_name, Last_name, Email, Address, Phone_no) VALUES('$username', '$password', '$fname', '$lname', '$email', '$address', '$phonenumber') ";
...
echo "              <div class=\"form-group row\">\n";
echo "               <div class=\"col-md-12\">\n";
echo "                 <input type=\"address\" name=\"address\" class=\"form-control\" placeholder=\"Address\">\n";
echo "               </div>\n";
echo "             </div>\n";
echo "		  \n";
echo "		  <div class=\"form-group row\">\n";
